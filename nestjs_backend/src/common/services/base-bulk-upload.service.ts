...
export abstract class BaseBulkUploadService {
    ...
    constructor(
        protected readonly uploadQueue: Queue,
    ...
    async initiateValidation(
        ...
        const uploadDir = path.join(process.cwd(), 'uploads', 'bulk', this.uploadType);
        if (!fs.existsSync(uploadDir)) {
            fs.mkdirSync(uploadDir, { recursive: true });
        }
        const ext = filename.split('.').pop();
        const filePath = path.join(uploadDir, `${this.uploadType}-upload-${upload.id}.${ext}`);
        ...
        this.logger.log(`${this.uploadType.toUpperCase()} validation initiated: ${upload.id} (Job: ${job.id}), saved to ${filePath}`);
    ...
    async confirmUpload(uploadId: string, userId: string): Promise<{ uploadId: string; jobId: string }> {
        ...
        this.logger.log(`${this.uploadType.toUpperCase()} import confirmed: ${upload.id} (Job: ${job.id})`);
    ...
    async cancelUpload(uploadId: string): Promise<void> {
        ...
        this.logger.log(`${this.uploadType.toUpperCase()} upload cancelled: ${uploadId}`);
    }
