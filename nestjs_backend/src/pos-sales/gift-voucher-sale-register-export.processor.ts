...
export interface GiftVoucherSaleRegisterExportJobData {
  jobId: string;
  userId: string;
  tenantId: string;
  tenantDbUrl: string;
...
const COLUMNS = [
  { header: 'Gift Voucher #', key: 'voucherNumber', width: 22 },
  { header: 'Type', key: 'voucherType', width: 14 },
...
export class GiftVoucherSaleRegisterExportProcessor {
  ...
  constructor(
    ...
    private readonly reportService: GiftVoucherSaleRegisterExportService,
  ) {
  ...
  async handleExport(job: Job<GiftVoucherSaleRegisterExportJobData>): Promise<void> {
    ...
    const fileName = `gift-voucher-sale-register-${new Date().toISOString().slice(0, 10)}.${ext}`;
  ...
  private async generateExcel(filePath: string, reportData: any): Promise<void> {
    ...
    const worksheet = workbook.addWorksheet('Gift Voucher Sale Register');
    ...
    for (const item of reportData.items) {
      const row = worksheet.addRow({
        ...
        baseInvoiceNumber: item.baseInvoiceNumber,
    ...
    const summaryRow = worksheet.addRow({
      ...
      amount: reportData.kpis.totalAmount,
      settledInInvoice: '-',
      settledDateTime: '-',
    });
    ...
    for (const item of reportData.items) {
      rowsHtml += `
        <tr>
          ...
          <td><span style="font-weight: bold; color: #475569;">${item.voucherType}</span></td>
          <td>${item.dateTime}</td>
          <td>${item.outletName}</td>
          <td>${item.customerDetail}</td>
          <td>${item.validTill}</td>
          <td style="font-family: monospace; color: #64748b;">${item.baseInvoiceNumber}</td>
          <td style="text-align: right;">PKR ${item.discountAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
          <td style="text-align: right; font-weight: bold; color: #059669;">PKR ${item.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
    ...
    return `
      ...
      <body>
        <div class="report-header">
          <h1 class="report-title">Gift Voucher Sale Register Report</h1>
          <div class="report-subtitle">Period: ${dateRangeStr}</div>
        </div>
        <table class="report-table">
          <thead>
            <tr class="header-row">
              <th style="width: 12%;">Gift Voucher #</th>
              <th style="width: 8%;">Type</th>
              <th style="width: 10%;">Date Time</th>
              <th style="width: 12%;">Issued Outlet</th>
              <th style="width: 14%;">Customer Detail</th>
              <th style="width: 8%;">Valid Till</th>
              <th style="width: 12%;">Base Inv / Claim #</th>
              <th style="width: 8%; text-align: right;">Discount</th>
              <th style="width: 8%; text-align: right;">Amount</th>
              <th style="width: 10%;">Settled In Inv</th>
              <th style="width: 10%;">Settled Date Time</th>
            </tr>
          </thead>
          <tbody>
            ${rowsHtml}
          ...
          <tfoot>
            <tr style="background: #0f172a; color: #fff; font-weight: bold;">
              <td colspan="7">TOTALS (${reportData.kpis.totalVouchers} Vouchers)</td>
              ...
              <td colspan="2">Settled Total: PKR ${reportData.kpis.totalSettledAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
            </tr>
          </tfoot>
        </table>
      </body>
