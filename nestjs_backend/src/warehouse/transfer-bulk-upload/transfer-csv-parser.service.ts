...
export interface TransferParsedRecord {
    row: number;
    data: {
        barCode?: string;
        sku?: string;
...
@Injectable()
export class TransferCsvParserService {
    private readonly logger = new Logger(TransferCsvParserService.name);
    ...
    private isEmptyRow(row: any): boolean {
        const barCode = this.getValue(row, ['BarCode', 'Barcode', 'bar_code', 'barCode']);
        const sku = this.getValue(row, ['SKU', 'sku', 'ItemCode', 'item_code']);
    ...
    private mapColumns(row: any): TransferParsedRecord['data'] {
        return {
            barCode: this.normalizeValue(this.getValue(row, ['BarCode', 'Barcode', 'bar_code', 'barCode'])) ?? undefined,
            sku: this.normalizeValue(this.getValue(row, ['SKU', 'sku', 'ItemCode', 'item_code'])) ?? undefined,
            quantity: this.parseNumber(this.getValue(row, ['Quantity', 'Qty', 'quantity', 'qty'])) ?? undefined,
        };
    }

    async parseCSVStreaming(fileBuffer: Buffer, onRecord: (record: TransferParsedRecord) => Promise<void>): Promise<void> {
    ...
    async parseExcelStreaming(fileBuffer: Buffer, onRecord: (record: TransferParsedRecord) => Promise<void>): Promise<void> {
        ...
        this.logger.log(`Processed ${rowCount} transfer records from Excel`);
    }

    async parseFileStreaming(fileBuffer: Buffer, filename: string, onRecord: (record: TransferParsedRecord) => Promise<void>): Promise<void> {
