...
async function main() {
  ...
  try {
    // 1. Fetch all unsynced sales orders
    ...
    console.log(`📋 Total Unsynced Invoices Found: ${unsyncedOrders.length}`);

    if (unsyncedOrders.length === 0) {
      console.log('✨ All sales orders are already synced with FBR!');
      return;
    }
    ...
    console.log('\n--- Unsynced Invoices Preview (Up to 50 shown) ---');
    console.table(tableData.slice(0, 50));
    if (tableData.length > 50) {
      console.log(`... and ${tableData.length - 50} more.`);
    }
