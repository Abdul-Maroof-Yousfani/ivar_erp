...
async function wipeLocationData(prisma: PrismaClient, options: WipeOptions) {
  ...
  const salesOrdersCount = await prisma.salesOrder.count({ where: { locationId: locId } });
  
  const posTerminals = await prisma.pos.findMany({ where: { locationId: locId }, select: { id: true } });
  const posIds = posTerminals.map(p => p.id);
  const posSessionsCount = posIds.length > 0 ? await prisma.posSession.count({ where: { posId: { in: posIds } } }) : 0;
