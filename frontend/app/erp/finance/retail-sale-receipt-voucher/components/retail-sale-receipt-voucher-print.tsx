...
export function RetailSaleReceiptVoucherPrint({
  voucher,
}: {
  ...
  const totalDebit =
    debitRows.reduce((s, d) => s + (Number(d.debit) || 0), 0) ||
    Number(voucher.debitAmount) ||
    0;
  const totalCredit =
    creditRows.reduce((s, d) => s + (Number(d.credit) || 0), 0) || totalDebit;

  // Extract Outlet Tag Code if present
  const outletTag = voucher.details.find(
    (d) => d.tagAccountCode || d.tagAccountName,
  );
                ...
                <div className="w-[45%] bg-[#f8fafc] text-[9px] sm:text-[10px] p-1 border border-gray-300 print:bg-[#f8fafc] [-webkit-print-color-adjust:exact] [color-adjust:exact] flex flex-col justify-center">
                  ...
                  <div className="flex justify-between">
                    <div className="flex gap-1.5">
                      ...
                      <span>
                        {voucher.rvDate
                          ? format(new Date(voucher.rvDate), "dd/MM/yyyy")
                          : ""}
                      </span>
                    </div>
                    <div className="flex gap-1.5">
                      ...
                      <span>
                        {voucher.folio ||
                          voucher.id.replace(/-/g, "").slice(-5).toUpperCase()}
                      </span>
                    </div>
                  </div>
        ...
        <tbody>
              ...
              >
                <td className="py-1 pr-1 align-top">
                  <div className="flex gap-1 sm:gap-1.5">
                    <span className="w-11 sm:w-14 shrink-0 font-bold">
                      {d.accountCode}
                    </span>
                    <span className="uppercase font-bold leading-tight">
                      {d.accountName}
                    </span>
                  </div>
                  {/* Tag Account */}
                  {(d.tagAccountCode || d.tagAccountName) && (
                    <div className="flex gap-1 sm:gap-1.5 mt-0.5">
                      <span className="w-11 sm:w-14 shrink-0 font-medium text-gray-700">
                        {d.tagAccountCode}
                      </span>
                      <span className="uppercase text-gray-700 leading-tight">
                        {d.tagAccountName}
                      </span>
                    </div>
                  )}
                ...
                <td className="py-1 pr-1 text-right tabular-nums align-top"></td>
              </tr>
            );
          })}
          ...
          <tr className="border-t-2 border-black">
            ...
            <td className="pt-1.5 pb-0.5 pr-1 text-right align-bottom">
              ...
              >
                <span className="tabular-nums text-[9px] sm:text-[10px] block pt-px font-bold whitespace-nowrap">
                  {fmt(totalDebit)}
                </span>
              </div>
            </td>
          ...
          <tr style={{ breakInside: "avoid", pageBreakInside: "avoid" }}>
            <td colSpan={4} className="pt-2 pb-0 font-normal text-left">
              ...
              <div className="mt-2 mb-1.5">
                ...
                <p className="text-[9px] mt-px text-gray-700">
                  {voucher.description}
                </p>
              </div>
