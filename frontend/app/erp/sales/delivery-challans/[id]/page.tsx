...
export default function DeliveryChallanViewPage() {
  ...
  const handleCancel = async () => {
    ...
    if (
      !confirm(
        "Are you sure you want to cancel this delivery challan? This will restore the inventory.",
      )
    ) {
      return;
    }
            ...
            <div className="flex items-center gap-2">
              ...
              {challan.status === "DELIVERED" && canCreateInvoice && (
                <Button size="sm" asChild>
                  <Link
                    href={`/erp/sales/invoices/create?challanId=${challan.id}`}
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div>
                    ...
                    {challan.salesOrder?.orderNo || "N/A"}
                  </div>
                  <div>
                    <span className="font-medium">Customer:</span>{" "}
                    {challan.customer?.name ||
                      challan.salesOrder?.customer?.name ||
                      "N/A"}
                  </div>
                  <div>
                    <span className="font-medium">Order Date:</span>{" "}
                    {challan.salesOrder?.orderDate
                      ? new Date(
                          challan.salesOrder.orderDate,
                        ).toLocaleDateString()
                      : "N/A"}
                  ...
                  <div>
                    ...
                    {formatCurrency(challan.salesOrder?.grandTotal || 0)}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div>
                    ...
                    {challan.vehicleNo || "N/A"}
                  </div>
                  <div>
                    <span className="font-medium">Transport Mode:</span>{" "}
                    {challan.transportMode || "N/A"}
                  </div>
                  <div>
                    <span className="font-medium">Status:</span>
                    <Badge
                      className={`ml-2 ${getStatusColor(challan.status || "PENDING")}`}
                    >
                      {challan.status || "PENDING"}
                    </Badge>
                  </div>
                    ...
                    <span>
                      {challan.totalQty ||
                        challan.items?.reduce(
                          (sum: number, item: any) =>
                            sum + (item.deliveredQty || item.quantity || 0),
                          0,
                        ) ||
                        0}
                    </span>
            ...
            <div className="flex gap-4 mb-4 text-xs sm:text-[13px]">
              <div className="w-1/2 p-2 border border-gray-300 flex flex-col justify-center">
                ...
                <div className="flex gap-2 mb-1">
                  <span className="font-bold w-16 shrink-0">Driver:</span>{" "}
                  <span>{challan.driverName || "N/A"}</span>
                </div>
                <div className="flex gap-2 mb-1">
                  <span className="font-bold w-16 shrink-0">Vehicle No:</span>{" "}
                  <span>{challan.vehicleNo || "N/A"}</span>
                </div>
                <div className="flex gap-2">
                  <span className="font-bold w-16 shrink-0">Transport:</span>{" "}
                  <span>{challan.transportMode || "N/A"}</span>
                </div>
              </div>
              <div className="w-1/2 p-2 border border-gray-300 flex flex-col justify-center">
                <div className="font-bold border-b border-gray-300 mb-2 pb-1">
                ...
                <div className="flex gap-2">
                  <span className="font-bold w-16 shrink-0">Order No:</span>{" "}
                  <span>{challan.salesOrder?.orderNo || "N/A"}</span>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-xs sm:text-[13px] mb-4 border-collapse table-fixed">
              <thead>
                <tr className="border-y-2 border-black">
                  <th className="py-2 pr-2 text-left font-bold w-[60%]">
                    Item Details
                  </th>
                  <th className="py-2 pr-2 text-right font-bold w-[20%]">
                    Unit Price
                  </th>
                  <th className="py-2 pr-2 text-right font-bold w-[20%]">
                    Delivered Qty
