...
export default function PurchaseReturnDetailPage() {
  ...
  const handleStatusUpdate = async (status: string) => {
    ...
    try {
      await purchaseReturnApi.updateStatus(
        purchaseReturn.id,
        status,
        "current-user-id",
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {purchaseReturn.status === "DRAFT" && (
                <PermissionGuard
                  permissions="erp.procurement.pret.update"
                  fallback={null}
                >
                  <>
                    <Link
                      href={`/erp/procurement/purchase-returns/${purchaseReturn.id}/edit`}
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {returnTypeLabels[purchaseReturn.returnType]}
                      </div>
                    </div>
                    <div>
              ...
              {purchaseReturn.sourceType === "GRN" && purchaseReturn.grn && (
                <div>
                  ...
                  <p className="text-gray-600">
                    GRN Number: {purchaseReturn.grn.grnNumber}
                  </p>
                  <p className="text-gray-600">
                    PO Number: {purchaseReturn.grn.purchaseOrder?.poNumber}
                  </p>
                </div>
              )}
              {purchaseReturn.sourceType === "LANDED_COST" &&
                purchaseReturn.landedCost && (
                  <div>
                    <h3 className="font-semibold">Landed Cost Reference</h3>
                    <p className="text-gray-600">
                      LC Number: {purchaseReturn.landedCost.landedCostNumber}
                    </p>
                    <p className="text-gray-600">
                      PO Number:{" "}
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  {purchaseReturn.debitNote && (
                    <Link
                      href={`/erp/procurement/debit-notes/${purchaseReturn.debitNote.id}`}
                      transitionTypes={["nav-forward"]}
                    >
                      <Button
                        size="sm"
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {formatDate(purchaseReturn.approvedAt!)}
                      </div>
                    </div>
                  </div>
                </CardContent>
            ...
            <div className="flex gap-4 mb-4 text-xs sm:text-[13px]">
              <div className="w-1/2 p-2 border border-gray-300 flex flex-col justify-center">
                ...
                <div className="flex gap-2 mb-1">
                  ...
                  <span>{purchaseReturn.supplier?.name || "N/A"}</span>
                </div>
              </div>
              <div className="w-1/2 p-2 border border-gray-300 flex flex-col justify-center">
                <div className="font-bold border-b border-gray-300 mb-2 pb-1">
                ...
                <div className="flex gap-2 mb-1">
                  <span className="font-bold w-16 shrink-0">Name:</span>{" "}
                  <span>{purchaseReturn.warehouse?.name || "N/A"}</span>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-xs sm:text-[13px] mb-4 border-collapse table-fixed">
              <thead>
                <tr className="border-y-2 border-black">
                  <th className="py-2 pr-2 text-left font-bold w-[40%]">
                    Item Details
                  </th>
                  <th className="py-2 pr-2 text-right font-bold w-[15%]">
                    Qty
                  </th>
                  <th className="py-2 pr-2 text-right font-bold w-[15%]">
                    Unit Price
            ...
            <div className="flex border-b border-black pb-2 items-end">
              <div className="w-[55%] pt-4">
                <div className="flex gap-2 font-bold text-xs sm:text-[13px]">
                  ...
                  <span className="underline decoration-1 underline-offset-2 break-words">
                    {numberToWords(Number(purchaseReturn.totalAmount || 0))}
                  </span>
                </div>
              </div>
              ...
              <div className="w-[20%] text-right">
                <div className="tabular-nums text-xs sm:text-[13px] text-gray-700">
                  {fmt(Number(purchaseReturn.subtotal || 0))}
                </div>
                <div className="tabular-nums text-xs sm:text-[13px] text-gray-700">
                  {fmt(Number(purchaseReturn.taxAmount || 0))}
                </div>
                <div
                ...
                >
                  <span className="tabular-nums font-bold text-xs sm:text-[13px] block pt-0.5">
                    {fmt(Number(purchaseReturn.totalAmount || 0))}
                  </span>
                </div>
              </div>
            </div>
