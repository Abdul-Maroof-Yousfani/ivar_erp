...
export function useUploadProgress(uploadId: string | null, uploadType: 'item' | 'item-update' | 'hscode' | 'employee' | 'attendance' | 'coa' | 'alliance' | 'sales-history' | 'online-sales' | 'stock' | 'merchant' | 'srn' | 'po' | 'direct-pi' | 'transfer' = 'item') {
    ...
    const getApiEndpoint = useCallback((endpoint: string) => {
        ...
        if (uploadType === 'direct-pi') {
            return `${baseUrl}/purchase-invoice/bulk-upload/${endpoint}`;
        }
