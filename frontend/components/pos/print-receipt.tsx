  ...
  return (
    <div className="font-mono text-xs w-full max-w-95 mx-auto space-y-2">
      ...
      {items.map((item: any, idx: number) => {
        ...
        const amtAfterDisc = totalWost - displayDisc;
        const tax = Math.round(amtAfterDisc * (taxPct / 100));
        const valueIncTax = amtAfterDisc + tax;
        const uniqueNo = item.sku || item.upc || "—";

        return (
          <div
            key={item.id ?? idx}
            className="py-1 border-b border-dashed last:border-0"
          >
            {/* Line 1: name grid */}
            {!isGiftReceipt ? (
              <div
            ...
            {/* Line 2: SKU + compact financials */}
            {!isGiftReceipt && (
              <div className="flex flex-wrap gap-x-2 text-[10px] text-muted-foreground mt-0.5">
                <span className="opacity-60">{uniqueNo}</span>
                {displayDiscPct > 0 && (
                  <span>
                    {suppressItemDiscounts ? "Alliance" : "Disc"}{" "}
                    {displayDiscPct}%/−{fmt(displayDisc)}
                  </span>
                )}
                <span>
                  Tax {taxPct}%/+{fmt(tax)}
      ...
      <div className="text-[10px] space-y-0.5">
        ...
        <p>
          • Damaged / defective items must be reported within 4 days of purchase
        </p>
        <p>• Exchanges are only accepted within 7 days of purchase</p>
        <p>• Refunds only eligible only on manufacturing defects</p>
        <p>
          • Used, washed, altered, or worn items are not eligible for exchanges
        </p>
        <p>• Tags must be intact for any exchange</p>
        <p>
          • One exchange per order only (exchanged item can not be exchanged
          again)
        </p>
        <p>
          • Sale items are non-returnable &amp; non-exchangeable (unless
          defected)
        </p>
      </div>
