    ...
    >
          ...
          >
            ...
            {(discAmt > 0 || taxPct > 0) && (
              <p className="text-[9px] text-zinc-500 leading-tight">
                {discAmt > 0 &&
                  `${isAllianceCase ? "Alliance Disc" : "Disc"}: -${fmtDec(discAmt)}`}
                {discAmt > 0 && taxPct > 0 && " | "}
            ...
            {line.priceAdjusted && (
              <p className="text-[8px] italic opacity-60">
                * Price adjusted: current lower than original
              </p>
            )}
      ...
      <div className="py-1 border-b border-zinc-400 space-y-0 text-[9px]">
        {(() => {
          ...
          const totalDiscountAmt = returnedLines.reduce(
          ...
          return (
            <>
              <Row
                label={`Subtotal Excl. Tax (${returnedLines.length})`}
                value={fmt(Math.round(subtotal))}
              />
              {totalDiscountAmt > 0 && (
                <Row
                  label="Total Discount"
                  value={`-${fmt(Math.round(totalDiscountAmt))}`}
                />
              )}
              <Row
              ...
              <Row
                ...
                value={fmt(Math.round(totalValueIncludingTax))}
                bold
              />
            </>
          );
        ...
        <div className="py-1 border-b border-dashed border-zinc-400 text-[9px] text-center space-y-0.5">
          ...
          <p>
            ...
            fmtExpiryDate(exchangeVoucher.expiresAt)
              ? ` · Exp: ${fmtExpiryDate(exchangeVoucher.expiresAt)}`
              : ""}
          </p>
      ...
      <div className="py-1 border-b border-dashed border-zinc-400 flex items-center gap-2">
        ...
        <p className="text-[8px] leading-tight flex-1">
          Return processed against FBR verified Sales Tax Invoice. Original
          invoice remains on record.
        </p>
      </div>
